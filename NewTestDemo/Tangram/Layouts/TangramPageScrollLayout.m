...
- (void)calculateLayout
{
    ...
    if(self.hasMoreAction.length > 0 && self.loadMoreImgUrl > 0 && self.scrollView.contentSize.width > self.vv_width)
    {
        ...
        __weak typeof(self) weakself = self;
        // FIXME: fixed
        [self.loadMoreImageView dtk_setImage:self.loadMoreImgUrl placeholder:nil completion:^(UIImage * _Nullable image, NSError * _Nullable error) {
            __strong typeof(weakself) strongSelf = weakself;
            if (!error && image)
            {
    ...
    if (self.bgImgURL.length > 0) {
        ...
        // FIXME: fixed
        [self.bgImageView dtk_setImage:self.bgImgURL placeholder:nil completion:nil];
...
- (void)buildIndicator
{
    ...
    if (self.indicatorImg1.length > 0) {
        // FIXME: fixed
        [self imageDownloaderWithUrl:self.indicatorImg1 completion:^(UIImage * _Nullable image, NSData * _Nullable data, NSError * _Nullable error) {
    ...
    if (self.indicatorImg2.length > 0) {
        /*
        [self imageDownloaderWithUrl:self.indicatorImg2 completion:^(UIImage * _Nullable image, NSData * _Nullable data, NSError * _Nullable error) {
            __strong typeof(self) strongSelf = weakSelf;
            if (!error) {
